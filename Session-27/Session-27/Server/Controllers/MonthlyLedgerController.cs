...
namespace Session_27.Server.Controllers {
    ...
    public class MonthlyLedgerController : ControllerBase {
        ...
        public async Task<List<MonthlyLedgerDto>> Get() {
            ...
            foreach (MonthlyLedgerDto monthlyLedger in monthlyLedgerList) {
                if (monthlyLedger.Month <= DateTime.Now.Month) {
                    MonthlyLedgerIncome(monthlyLedger, trans);
                    MonthlyLedgerExpenses(monthlyLedger, engineers, managers);
